import React from "react";

const Investments = () => {
  return (
      <div>
          <div className="investment">
      <img className="investify"
        src="https://storage.googleapis.com/piggyvestwebsite/piggywebsite2020/invest_c3fcc60df0/invest_c3fcc60df0.png"
       
      />
      <button className="returns">Up To 25% Returns</button>
      <h1 className="access">Access investment opportunities</h1>
      <p className="advice">
        Invest securely and confidently on the go. Grow your money confidently
        by investing in pre-vetted investment opportunities.
      </p>
      <button className="learn">Learn more about Investments > </button>

      </div>
  
    
  
    </div>
  );
};

export default Investments;
